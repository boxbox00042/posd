...
#include "../../src/iterator/factory/bfs_step_iterator_factory.h"

class BFSStepCompoundIteratorTest : public ::testing::Test
{
protected:
    Point *p1, *p2, *p3, *p4;
...
TEST_F(BFSStepCompoundIteratorTest, CurrentItemShouldBeCorrect)
{
    ASSERT_NEAR(3 * 3 * M_PI, it->currentItem()->area(), 0.001);
}

TEST_F(BFSStepCompoundIteratorTest, NextShouldBeCorrect)
{
    it->next();
    ASSERT_NEAR(5 * 5 * M_PI + 25, it->currentItem()->area(), 0.001);
}

TEST_F(BFSStepCompoundIteratorTest, IsDoneShouldBeCorrect)
{
    it->next();
    it->next();
    it->next();
...
TEST_F(BFSStepCompoundIteratorTest, CurrentItemShouldThrowExceptionWhenIsDone)
{
    it->next();
    it->next();
    it->next();
...
TEST_F(BFSStepCompoundIteratorTest, NextShouldThrowExceptionWhenIsDone)
{
    it->next();
    it->next();
    it->next();
...
TEST_F(BFSStepCompoundIteratorTest, BFSOrderShouldBeCorrectIfNoChildrenInCompound)
{
    Shape *sps[] = {};
...
TEST_F(BFSStepCompoundIteratorTest, BFSOrderShouldBeCorrectIfACircleInCompound)
{
...
TEST_F(BFSStepCompoundIteratorTest, BFSOrderShouldBeCorrectWithMultipleCompoundShapes)
{
    Shape *sps1[] = {};
    Shape *sps2[] = {};
...
TEST_F(BFSStepCompoundIteratorTest, BFSOrderShouldBeCorrectWithComplicatedTreeStructure)
{
